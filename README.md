...
- **Claude Code（推奨）** — `.claude/skills/` の Skill を自動トリガーして CLI を呼び出す
- **Cursor / Codex CLI / Gemini CLI など** — Skill ファイルを各エージェント固有のパスに配置すれば自動トリガー可能（[後述](#cursor--codex-cli--gemini-cli-で使う)）。配置しない場合も `AGENTS.md` 経由で CLI を呼ばせられる
...
Claude Code でこのリポジトリを開くと、Agent Skills が自動で有効になります。自然言語でリクエストすれば、Skill が必要な CLI コマンドを組み立てて実行します。
...
### Cursor / Codex CLI / Gemini CLI で使う

Skill 自体は仕様の標準化が進んでいますが、現時点で `.claude/skills/` を自動スキャンするのは Claude Code のみです。他のエージェントで Skill を自動トリガーさせたい場合は、各エージェントが見るパスにコピーまたはシンボリックリンクを張ります。
...
Claude Code でリポジトリを開くと自動的にトリガーされる Skill を7つ搭載しています。Skill はモデルへの指示書であり、CLI コマンドを組み合わせて分析や取引を実行します。
あくまでサンプルですので、ご自身の用途に合わせて追加・編集してください。Cursor / Codex CLI / Gemini CLI など他エージェントでの配置先は [使い方](#cursor--codex-cli--gemini-cli-で使う) を参照してください。

...
describe("create-order", () => {
  it("returns dryRun when --execute is not set", async () => {
    ...
    const result = await createOrder({
      pair: "btc_jpy", side: "buy", type: "limit",
      price: "5000000", amount: "0.001",
    });
  ...
  it("calls API when --execute is set", async () => {
    const result = await createOrder(
      { pair: "btc_jpy", side: "buy", type: "limit", price: "5000000", amount: "0.001", execute: true },
      { fetch: mockFetch(VALID_RESPONSE), retries: 0, credentials: CREDS, nonce: "1" },
    );
    expect(result.success).toBe(true);
    if (result.success) expect((result.data as any).order_id).toBe(123);
  });

  it("validates price required for limit order", async () => {
    const result = await createOrder({
      pair: "btc_jpy", side: "buy", type: "limit", amount: "0.001",
    });
  ...
  it("validates trigger-price required for stop_limit", async () => {
    const result = await createOrder({
      pair: "btc_jpy", side: "sell", type: "stop_limit",
      price: "5000000", amount: "0.001",
    });
    expect(result.success).toBe(false);
  ...
  it("validates amount > 0", async () => {
    const result = await createOrder({
      pair: "btc_jpy", side: "buy", type: "market", amount: "0",
    });
    expect(result.success).toBe(false);
  ...
  it("validates side enum", async () => {
    const result = await createOrder({
      pair: "btc_jpy", side: "invalid", type: "market", amount: "0.001",
    });
    expect(result.success).toBe(false);
  });

...
description: |
  bitbank の暗号資産ローソク足データから、複数銘柄間の関係性を定量化する。
  リターン系列ベースで Pearson / Spearman 相関、β 行列、ローリング相関、
  外れ値トリミング、環境別（上昇局面 / 下落局面）相関、ラグ相関
  （クロスコリログラム）を計算し、「BTC と ETH はどれくらい連動するか」
  「ETH の β は」「下落時に相関は上がるか」「銘柄間にリードラグはあるか」
  といった問いに記述的に答える。
  「BTC-ETH の相関は」「分散投資効果はある？」「ETH の β は？」
  「環境別相関を見て」「銘柄間ラグはある？」「アルトの連動度は？」
  のような発話で起動する。曖昧な「BTC と ETH ってどれくらい連動してる？」
  「複数銘柄の関係性を見て」もカバーする。
  注意: 本 skill は記述的分析であり、売買判断は出さない。

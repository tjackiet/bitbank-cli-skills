...
| `trade withdraw` | 出金リクエスト（人間の対話実行のみ／AI エージェント起動は非推奨） | `trade withdraw --asset=btc --uuid=xxx --amount=0.1 --execute --confirm` |

> Trade コマンドは `--execute` を付けない限り API を叩きません（ドライラン）。`withdraw` は追加で `--confirm` も必須です。サブコマンド一覧は `bitbank trade` で表示できます。
>
> **`trade withdraw` の運用方針**: 安全ガード（dry-run + `--execute` + `--confirm` + 対話確認 + POST 再送無効）は十分に堅牢ですが、本 PoC では **AI エージェント経由での起動は非推奨** とします。出金は人間が CLI から直接実行する用途に限定してください。Skill / MCP からの自動起動フローを組まないこと。read-only な `withdrawal-accounts` / `withdrawal-history` は AI 経由で利用して問題ありません。
>

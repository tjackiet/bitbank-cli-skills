...
| カテゴリ | ディレクトリ | 認証 | 説明 |
|---------|------------|------|------|
| public | `cli/commands/public/` | 不要 | 公開マーケットデータ（ticker, candles 等） |
| private | `cli/commands/private/` | 必要 | アカウント情報の読み取り（assets, orders 等） |
| trade | `cli/commands/trade/` | 必要 | 資金に影響する操作（create-order, withdraw 等） |
...
6. `cli/__tests__/` にテストを追加
7. 1 ファイル 100 行以内。超えたら分割

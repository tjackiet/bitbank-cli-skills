...
description: |
  任意のシグナル候補（テクニカル指標、出来高変換量、自作特徴量、外部時系列）の
  予測力を客観的に評価する。生データ vs 将来リターン相関、ローリング Z-score
  変換による改善度、ラグ相関プロファイル、自己相関（粘着性）、既存指標との
  冗長性、コスト無視の符号ベース簡易 PnL、リーク自己チェックを通して、
  シグナルが「本当に効くか」を数値で示す。
  「RSI、本当に効く？」「この指標に予測力ある？」「自作のシグナルを評価して」
  「BB 幅は将来リターンと相関ある？」「Z-score 化で改善する？」
  「シグナル候補を評価して」のような発話で起動する。
  注意: 「BTC の RSI を見て」（現在値の読み）は indicator-analysis、

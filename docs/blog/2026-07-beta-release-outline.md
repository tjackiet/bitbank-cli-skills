...
技術詳細ではなく「思想」を伝える。
**軸は「AI エージェントに触ってもらうことを前提に、ガードレールをしっかり目に作ってある」**:

- 操作者が人間ではなく AI になると、誤解釈やプロンプトの誤コピーで「意図しない発注」が起き得る。
  だからお金が動く操作には、AI が文脈の勢いでは通せない形の確認を必ず挟む
  - MCP: preview → execute の 2 ステップ + ホスト側の承認ダイアログ
    （取引ツールは「常に許可（Always allow）」にしない運用も明記）
  - CLI: デフォルトはドライラン。`--execute` に加えて固定フレーズ
    `--confirm=I-UNDERSTAND-...` が揃って初めて実行される二段ロック
- **出金はそもそも実装しない**（出金権限を付ける理由自体をなくす）
- ローカル完結。API キーは利用者自身が発行・管理し、
  A → B → C のはしごで「AI に渡す権限」を自分で選ぶ（最小権限: 「参照」のみ推奨）

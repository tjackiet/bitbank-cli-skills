...
- 本ツールが提供するデータを AI エージェント（Agent Skills）が処理した結果は、必ずしも正確性・完全性を保証するものではありません。
- 提供される情報は情報提供のみを目的としており、**投資助言・代理業に該当するものではありません**。
- 暗号資産の取引にはリスクが伴います。**投資に関する判断はご自身の責任**で行ってください。

...
このプロジェクトは npm パッケージとしては配布していません。**リポジトリをクローンして、Claude Code などのエージェント環境から自然言語で操作する**のが基本スタイルです。

- **Claude Code（推奨）** — Agent Skills が CLI を自動で組み合わせて分析・取引を実行
- **Codex などその他のエージェント** — `AGENTS.md` を読ませて CLI を呼ばせる（Skill の自動トリガーは Claude Code 限定ですが、CLI 自体は汎用）
- **ターミナル** — 動作確認やスクリプト連携、cron との接続など

Skills は利用者が自由に追加・編集していく前提なので、リポジトリごと手元に置く必要があります。
...
### Codex などその他のエージェントで使う

ルートの `AGENTS.md` をエージェントに読ませれば、CLI を呼ばせて分析できます。Skill の自動トリガーは Claude Code 専用ですが、CLI 自体はどのエージェントからも使えます。

...
Cursor は互換のため `.claude/skills/` をそのまま読むので、追加作業なしで Claude Code と同様に使えます。Codex CLI / Gemini CLI など他のエージェントで Skill を自動トリガーさせたい場合は、各エージェントが見るパスにコピーまたはシンボリックリンクを張ります。

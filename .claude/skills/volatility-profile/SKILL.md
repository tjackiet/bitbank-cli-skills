...
description: |
  bitbank の暗号資産ローソク足データから、市場のリスク特性を定量化する。
  リターン分布の基本統計量（歪度・超過尖度）、ファットテール倍率、
  時間帯別出来高プロファイル、出来高 × |リターン| の相関、
  √T スケーリング比を計算し、ストップ幅・ポジションサイジング・
  低流動性時間帯回避といった実務判断の材料を提示する。
  「BTC のボラどう？」「ボラ特性見て」「ファットテール度は？」
  「±3σ は理論の何倍出る？」「ストップ幅どう決める？」
  「出来高薄い時間帯は？」「リスク特性を見て」のような質問で起動する。
  曖昧な「リスクどうなってる？」「分布見て」もカバーする。
